...
pte_dbow = models.base_model.Model(
    tag="PTransE hyperparams Paragraph Vector - Distributed Bag of Words",
    distributed_memory=False,
    epochs=500,
    learning_rate=0.001,
    dimensions=100,
    write_out_dir="../../../honours-data-round-2/FB15K/model_out/evaluation/pte_pv_dbow"
)
...
pte_dbow.build_vocab(sentences)
pte_dbow.train(sentences)

pte_dbow.save_vectors_in_memory_and_delete_model()
...
pte_dbow.parse_relation_types(n2n_file_path=N2N_FILE)

triple_star_map = [[t, True] for t in test_triples]
pte_dbow.evaluate(triple_star_map)
