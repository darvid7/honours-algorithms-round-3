...
d2v_dm = models.base_model.Model(
    tag="Doc2Vec suggested hyperparams Paragraph Vector - Distributed Memory",
    distributed_memory=True,
    epochs=100,
    learning_rate=0.005,
    dimensions=100,
    write_out_dir="../../../honours-data-round-2/FB15K/model_out/evaluation/middle_pv_dm"
)

# Things to read training data.
DATA_DIR = "../../"
PATH_DIR = "dataset_generation"
PATH_FILE_SUFFIX = "as_ints.txt"  # Use a v smalimport os
import time
import pickle
...
d2v_dm.build_vocab(sentences)
d2v_dm.train(sentences)

d2v_dm.save_vectors_in_memory_and_delete_model()
...
d2v_dm.parse_relation_types(n2n_file_path=N2N_FILE)

triple_star_map = [[t, True] for t in test_triples]
d2v_dm.evaluate(triple_star_map)
