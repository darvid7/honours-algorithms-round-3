...
def main(_):
    ...
    if (config.testFlag):
        ...
        print("config.nbatches: %s" % config.nbatches)
        print("triple total: %s" % lib.getTripleTotal())
        import sys
        sys.exit(-1)
    ...
    with tf.Graph().as_default():
        ...
        with sess.as_default():
            ...
            if (config.loadFromData):
                saver.restore(sess, './transEmodelTest/model.ckpt')
            ...
            if not config.testFlag:
                for times in range(config.trainTimes):
                    ...
                    print(res)
                saver.save(sess,'./transEmodelTest/model.ckpt')
            else:
