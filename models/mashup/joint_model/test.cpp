...
string inPath = "../../../hons-data-trans-e-x-format/";
...
void init() {
    ...
    printf("test.cpp TEST TOTAL %d\n", testTotal);
    tmp = fscanf(f_kb2, "%d", &validTotal);
    printf("test.cpp TRAIN TOTAL %d\n", validTotal);
    tmp = fscanf(f_kb3, "%d", &validTotal);
    printf("test.cpp VALID TOTAL %d\n", validTotal);
    ...
    for (int i = 0; i < testTotal; i++) {
        ...
        label++; // This is just to make it not 0 indexed i think.
        nntotal[label]++;
...
void getHeadBatch(int *ph, int *pt, int *pr) {
	...
	// printf("Test triple (h, t, r) %d %d %d\n", testList[lastHead].h, testList[lastHead].t, testList[lastHead].r);
}
...
void test() {
    ...
    for (int i = 1; i <= 4; i++) {
        ...
        printf("left(filter) %f %f %f %f \n",
            l_filter_rank[i] / nntotal[i],
            l10_filter_tot[i] / nntotal[i],
            l3_filter_tot[i] / nntotal[i],
            l1_filter_tot[i] / nntotal[i]);
        ...
        printf("right %f %f %f %f \n",
            ...
            r3_tot[i] / nntotal[i],
             r1_tot[i] / nntotal[i]);
        printf("right(filter) %f %f %f %f\n",
            r_filter_rank[i] / nntotal[i],
            r10_filter_tot[i] / nntotal[i],
            r3_filter_tot[i] / nntotal[i],
            r1_filter_tot[i] / nntotal[i]);
    }
}
