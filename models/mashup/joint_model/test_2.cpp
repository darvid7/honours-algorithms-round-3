...
//        inPath = inPath + path[i];
    printf("Input Files Path (dlei test) called but is buggy: %s\n", inPath.c_str());
}
...
extern "C"
void init() {
    printf("test.cpp initing()\n");
    FILE *fin;
    printf("test.cpp initing\n");
    int tmp, h, r, t, label;
	fin = fopen((inPath + "relation2id.txt").c_str(), "r");
    tmp = fscanf(fin, "%d", &relationTotal);
    printf("read relation2id.txt\n");;
    fclose(fin);
	fin = fopen((inPath + "entity2id.txt").c_str(), "r");
    tmp = fscanf(fin, "%d", &entityTotal);
    fclose(fin);
    printf("read in entity2id.txt\n");
