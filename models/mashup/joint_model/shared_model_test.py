...
class Config(object):
    ...
    def __init__(self):
        # lib.setInPath(os.path.join(p, "./data/FB15K/"))
        # test_lib.setInPath(os.path.join(p, "./data/FB15K/"))
        lib.setBernFlag(0)
...
def main(_):
    start = time.time()
    shared_model_emd_data_dir = './shared_transE_half/'
    config = Config()
    test_lib.init()
    print("test init after init")
    ...
    with tf.Graph().as_default():
        ...
        with sess.as_default():
            ...
            if (config.loadFromData):
                print("Loading shared model data from %s" % shared_model_emd_data_dir)
                saver.restore(sess, shared_model_emd_data_dir + 'model.ckpt')
            ...
            # Sometimes the python gets outpptued before the cpp due to idk so need to flush?
            print("Final results after %s tests" % total)
