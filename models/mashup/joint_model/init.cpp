...
extern "C"
void setInPath(char *path) { // Is buggy, just hardcode inpath.
//    int len = strlen(path);
//    inPath = "";
//    for (int i = 0; i < len; i++)
//        inPath = inPath + path[i];
    printf("Input Files Path (dlei init): %s Called but is buggy\n", inPath.c_str());
}
